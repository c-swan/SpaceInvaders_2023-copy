...
bool Game::CheckCollision(Vector2 circlePos, float circleRadius, Vector2 lineStart, Vector2 lineEnd) {
	if (pointInCircle(circlePos, circleRadius, lineStart)
	    || pointInCircle(circlePos, circleRadius, lineEnd)) {
		return true;
	}
	// simplify variables
	Vector2 A = lineStart;
	...
	float dotProduct = (((C.x - A.x) * (B.x - A.x)) + ((C.y - A.y) * (B.y - A.y))) / (length * length);
	Vector2 closestPoint = A + dotProduct * (B - A);
	// find out if coordinates are on the line by comparing the distance of the dot to the edges, with two vectors
	// if the distance of the vectors combined is the same as the length the point is on the line
	// since we are using floating points, we will allow the distance to be slightly innaccurate to create a smoother collision
	float buffer = 0.1f;
	float closestLength = lineLength(A, closestPoint) + lineLength(B, closestPoint);

	if (closestLength == length + buffer || closestLength == length - buffer) { //Point is on the line!
		return lineLength(A, closestPoint) < circleRadius;
	}
	return false;
}
