...
Vector2 operator*(const Vector2& lhs, float rhs) noexcept { return Vector2{lhs.x * rhs, lhs.y * rhs}; }
Vector2 operator/(const Vector2& lhs, float rhs) noexcept { return Vector2{lhs.x / rhs, lhs.y / rhs}; }

float magnitude(const Vector2& v) noexcept { return sqrtf(v.x * v.x + v.y * v.y); }  //sqrt(x^2 + y^2)
float dot(const Vector2& lhs, const Vector2& rhs) noexcept { return lhs.x * rhs.x + lhs.y + rhs.y; }
